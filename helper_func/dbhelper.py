...
class Database:
    ...
    def get_video(self, user_id: int) -> str:
        """Compat wrapper for newer code paths."""
        return self.get_vid_filename(user_id)

    def get_sub(self, user_id: int) -> str:
        """Compat wrapper for newer code paths."""
        return self.get_sub_filename(user_id)

    def set_video(self, user_id: int, vid_name: str, filename: str | None = None):
        """
        Compat wrapper: newer code may call set_video(user_id, vid_name, original_name?).
        Falls back to vid_name if a separate original filename isn't provided.
        """
        return self.put_video(user_id, vid_name, filename or vid_name)

    def set_sub(self, user_id: int, sub_name: str):
        """Compat wrapper for newer code paths."""
        return self.put_sub(user_id, sub_name)
